...
function NavItemLink({ item, pathname, collapsed }: { item: NavItem; pathname: string; collapsed: boolean }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ flexShrink: 0, color: active ? "var(--text-primary, #fafafa)" : "var(--text-muted, #9ca3af)", transition: "color 0.15s" }}>
          {item.icon}
        </span>
        {!collapsed && (
          <span
            style={{
              fontSize: 13,
              fontWeight: 500,
              color: active ? "var(--text-primary, #fafafa)" : "var(--text-muted, #9ca3af)",
              whiteSpace: "nowrap",
              overflow: "hidden",
...
export default function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        {expanded ? (
          <span style={{ fontSize: 14, fontWeight: 700, letterSpacing: "0.15em", color: "var(--text-primary, #fafafa)" }}>
            TRADEZEN
          </span>
        ) : (
          <span style={{ fontSize: 14, fontWeight: 700, letterSpacing: "0.15em", color: "var(--text-primary, #fafafa)", width: "100%", textAlign: "center" }}>
            TZ
          </span>
        )}
        {expanded && (
          <button
            ...
            style={{
              ...
              border: "none",
              cursor: "pointer",
              color: pinned ? "var(--accent, #3b82f6)" : "var(--text-muted, #9ca3af)",
              padding: 4,
              borderRadius: 4,
      ...
      <nav style={{ flex: 1, overflowY: "auto", overflowX: "hidden", paddingTop: 8 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {GROUP1.map((item) => (
            <NavItemLink key={item.href} item={item} pathname={pathname} collapsed={!expanded} />
          ))}
        </div>

        {/* Divider */}
        <div style={{ height: 1, background: "var(--border, #23252d)", margin: "12px 16px" }} />
      ...
      {/* User */}
      <div style={{ borderTop: "1px solid var(--border, #23252d)", flexShrink: 0 }}>
        <div
          style={{
            display: "flex",
            alignItems: "center",
        ...
        >
          ...
          {expanded && (
            <div style={{ flex: 1, overflow: "hidden" }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-primary, #fafafa)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
