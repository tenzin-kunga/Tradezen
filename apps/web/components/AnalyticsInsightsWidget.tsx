...
function SmallSampleBadge() {
  return (
    <span
      className="inline-flex items-center px-1.5 py-0.5 text-[10px] leading-tight rounded"
      style={{
        background: "var(--glass-bg-alt, rgba(255,255,255,0.06))",
        color: "var(--text-dim)",
      }}
    >
      small sample
...
function renderMetrics(
  ...
  switch (category) {
    ...
    case "discipline": {
      ...
      const fomoPct =
        behavioral.fomoCount && totalTrades
          ? Math.round((behavioral.fomoCount / totalTrades) * 100)
          : 0;
      const vengeancePct =
        behavioral.vengeanceCount && totalTrades
          ? Math.round((behavioral.vengeanceCount / totalTrades) * 100)
          : 0;
      push("FOMO Entries", `${fomoPct}%`, totalTrades);
      push("Revenge Trades", `${vengeancePct}%`, totalTrades);
      push(
        "Trend Alignment",
        behavioral.trendAlignedCount && totalTrades
          ? `${Math.round((behavioral.trendAlignedCount / totalTrades) * 100)}%`
          : "--",
        totalTrades,
      );
      break;
    ...
    case "risk": {
      ...
      push(
        "Profit Factor",
        profitFactor > 0 && profitFactor < 999
      ...
      push(
        "Avg R:R",
        analytics.avgRR > 0 ? `1:${analytics.avgRR.toFixed(1)}` : "--",
      );
      ...
      push(
        ...
        analytics.bestTrade > 0
          ? `+$${Number(analytics.bestTrade).toFixed(0)}`
          : "--",
      );
      push(
        "Largest Loss",
    ...
    case "consistency": {
      ...
      push(
        ...
        streak?.count && streak.count > 0
          ? `${streak.type === "win" ? "🔥" : "❄️"} ${streak.count} trades`
          : "No active streak",
      );
