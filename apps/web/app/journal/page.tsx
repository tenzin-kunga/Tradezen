...
export default function JournalPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#111111] text-gray-500 font-mono flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen text-white font-mono p-4 md:p-6 lg:p-10">
      {/* Header */}
        ...
        <div className="flex gap-2">
          {(["editor", "history"] as const).map((v) => (
            <button key={v} onClick={() => setView(v)}
              className={`px-4 py-2 font-mono text-xs font-bold tracking-widest rounded border border-[#2a2a2a] cursor-pointer transition-colors ${
                view === v ? "bg-white text-[#111]" : "bg-transparent text-gray-500"
              }`}
            >
              {v.toUpperCase()}
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
        {[
          ...
          <div key={s.label} className={`${sectionCls} mb-0 text-center`}>
            <div className="text-xs tracking-widest mb-2" style={{ color: "#555" }}>{s.label}</div>
      ...
      {view === "editor" ? (
        <>
          {/* Date picker */}
          <div className={`${sectionCls} flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4`}>
            <div>
              <label className={labelCls}>JOURNAL DATE</label>
              <input
            ...
            <div className="flex gap-2">
              <button onClick={() => { const d = new Date(selectedDate); d.setDate(d.getDate() - 1); setSelectedDate(toDateStr(d)); }}
                className="bg-transparent border border-[#2a2a2a] text-gray-500 px-3 py-2 cursor-pointer font-mono text-xs rounded">
                ← PREV
              </button>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {moods.map((m) => (
                <button key={m.value} onClick={() => setMood(m.value)}
                  ...
                  style={{
                    backgroundColor: mood === m.value ? `${m.color}22` : "#111",
                    border: `1px solid ${mood === m.value ? m.color : "#2a2a2a"}`,
                    color: mood === m.value ? m.color : "#555",
                  }}
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className={sectionCls}>
              <label className={labelCls}>PRE-MARKET NOTES</label>
              <textarea
            ...
            <div className={sectionCls}>
              <label className={labelCls}>POST-MARKET NOTES</label>
              <textarea
                className={`${inputCls} min-h-[120px] resize-y`}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className={sectionCls}>
              <label className={labelCls}>MARKET CONDITIONS</label>
              <textarea
            ...
            <div className={sectionCls}>
              <label className={labelCls}>KEY LESSONS</label>
              <textarea
                className={`${inputCls} min-h-[80px] resize-y`}
          ...
          <div className={`${sectionCls} flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4`}>
            <div>
              {currentEntry ? (
                <button onClick={handleDelete}
                  className="bg-transparent border border-red-500/25 text-red-500 px-5 py-2.5 cursor-pointer font-mono text-xs font-bold tracking-widest rounded">
                  DELETE ENTRY
                </button>
              ) : (
            ...
            <button onClick={handleSave} disabled={saving}
              className={`px-8 py-3 font-mono text-xs font-bold tracking-widest rounded ${saving ? "bg-[#333] text-gray-500 cursor-not-allowed" : "bg-white text-[#111] cursor-pointer"}`}>
        ...
        /* History View */
        <div className={sectionCls}>
          <div className="text-xs text-gray-600 tracking-widest mb-5">
            JOURNAL HISTORY — {entries.length} ENTRIES
          </div>
          {entries.length === 0 ? (
            <div className="text-sm text-center py-10" style={{ color: "#555" }}>
              ...
              {entries.map((e) => {
                ...
                return (
                  <button
                    ...
                    className="flex justify-between items-center bg-[#111] border border-[#2a2a2a] rounded px-4 py-3.5 cursor-pointer text-left w-full font-mono transition-colors hover:border-[#555]"
                  >
                    ...
                    <div className="text-xs tracking-wide font-bold" style={{ color: moodObj?.color || "#555" }}>
                      {moodObj?.label || "—"}
                    </div>
                  </button>
                );
