  ...
  return (
    <div>
      ...
      <div style={{ fontSize: "11px", color: "#888", marginTop: "8px", display: "flex", flexDirection: "column", gap: "4px" }}>
        <span>RISK AMT: <span style={{ color: "#fff" }}>{risk.toFixed(4)}</span></span>
        <span>MAX REWARD: <span style={{ color: "#22c55e" }}>{reward.toFixed(4)}</span></span>
      </div>
    </div>
  );
}
...
function TogglePill({ label, value, onChange }: { label: string; value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ display: "flex", alignItems: "center", gap: "10px", background: "none", border: "none", cursor: "pointer", padding: "6px 0" }}
    >
      <div
        style={{
      ...
      >
        <div
          style={{
            ...
            backgroundColor: value ? "#111111" : "#888888", transition: "left 0.15s",
          }}
        />
      </div>
      <span style={{ fontSize: "11px", letterSpacing: "0.1em", color: value ? "#ffffff" : "#888888" }}>{label}</span>
    </button>
  );
}
...
const inputStyle: React.CSSProperties = {
  ...
  padding: "10px 12px", color: "#ffffff", fontFamily: "monospace", fontSize: "13px", outline: "none", boxSizing: "border-box",
};

const labelStyle: React.CSSProperties = { fontSize: "10px", color: "#888", letterSpacing: "0.12em", marginBottom: "6px", display: "block" };
...
export default function EditTradePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!symbol.trim()) { setError("INSTRUMENT TICKER is required."); return; }
    if (!entry || !exit || !quantity) { setError("Entry price, exit price, and quantity are required."); return; }
  ...
  if (loadingTrade) {
    return (
      <div style={{ minHeight: "100vh", backgroundColor: "#111111", color: "#555", fontFamily: "monospace", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: "12px", letterSpacing: "0.15em" }}>LOADING TRADE DATA...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#111111", color: "#ffffff", fontFamily: "monospace" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "32px" }}>
        <div>
          <h1 style={{ fontSize: "22px", fontWeight: 700, letterSpacing: "0.1em", margin: 0 }}>EDIT EXECUTION</h1>
        ...
        <button
          ...
          onClick={() => router.back()}
          style={{
            background: "none", border: "1px solid #2a2a2a", borderRadius: "4px", color: "#888",
            cursor: "pointer", fontFamily: "monospace", fontSize: "11px", letterSpacing: "0.1em", padding: "8px 16px",
          }}
        >
          ← CANCEL
        </button>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: "16px" }}>
          ...
          <div>
            ...
            <div style={sectionStyle}>
              <div style={{ fontSize: "10px", color: "#555", letterSpacing: "0.15em", marginBottom: "16px" }}>01 // INSTRUMENT PARAMETERS</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
                <div>
                  <label style={labelStyle}>INSTRUMENT TICKER</label>
                  <input style={inputStyle} value={symbol} onChange={(e) => setSymbol(e.target.value)} placeholder="e.g. BTCUSD" autoComplete="off" />
                </div>
                <div>
                  ...
                  <div style={{ display: "flex", gap: "0" }}>
                    {(["LONG", "SHORT"] as const).map((d) => (
                      <button
                        ...
                        style={{
                          flex: 1, padding: "10px", fontFamily: "monospace", fontSize: "12px", fontWeight: 700,
                          letterSpacing: "0.1em", border: "1px solid #2a2a2a", cursor: "pointer",
            ...
            <div style={sectionStyle}>
              <div style={{ fontSize: "10px", color: "#555", letterSpacing: "0.15em", marginBottom: "16px" }}>02 // EXECUTION METADATA</div>
            ...
            <div style={sectionStyle}>
              <div style={{ fontSize: "10px", color: "#555", letterSpacing: "0.15em", marginBottom: "16px" }}>03 // BEHAVIORAL FLAGS</div>
          ...
          <div>
            <div style={{ ...sectionStyle, position: "sticky", top: "40px" }}>
              <div style={{ fontSize: "10px", color: "#555", letterSpacing: "0.15em", marginBottom: "16px" }}>LIVE R:R CALCULATOR</div>
              <div style={{ marginBottom: "20px" }}>
                <div style={{ fontSize: "10px", color: "#888", letterSpacing: "0.1em", marginBottom: "8px" }}>RISK/REWARD RATIO</div>
                <RRDisplay entry={entry} stopLoss={stopLoss} takeProfit={takeProfit} />
              </div>

              {/* Confidence bars */}
              <div style={{ borderTop: "1px solid #2a2a2a", paddingTop: "16px" }}>
                <div style={{ fontSize: "10px", color: "#555", letterSpacing: "0.12em", marginBottom: "12px" }}>CONFIDENCE METRICS</div>
                {[
                  ...
                  <div key={bar.label} style={{ marginBottom: "10px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                      ...
                      <span style={{ fontSize: "10px", color: bar.pct >= 70 ? "#22c55e" : bar.pct >= 40 ? "#e8603c" : "#ef4444" }}>{bar.pct}%</span>
                    </div>
                    <div style={{ height: "3px", backgroundColor: "#2a2a2a", borderRadius: "2px" }}>
                      <div style={{ height: "3px", borderRadius: "2px", width: `${bar.pct}%`, backgroundColor: bar.pct >= 70 ? "#22c55e" : bar.pct >= 40 ? "#e8603c" : "#ef4444", transition: "width 0.3s" }} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div
          style={{
            backgroundColor: "#1c1c1c", border: "1px solid #2a2a2a", borderRadius: "4px",
            padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "16px",
          }}
        >
          {error ? (
            <span style={{ fontSize: "12px", color: "#ef4444", letterSpacing: "0.05em" }}>{error}</span>
          ) : (
            <span style={{ fontSize: "11px", color: "#555", letterSpacing: "0.08em" }}>MODIFY MODE // CHANGES WILL OVERWRITE</span>
          )}
          <button
            type="submit" disabled={submitting}
            style={{
              backgroundColor: submitting ? "#333" : "#ffffff", color: submitting ? "#888" : "#000000",
              border: "none", borderRadius: "4px", padding: "12px 32px", fontFamily: "monospace",
              fontSize: "12px", fontWeight: 700, letterSpacing: "0.15em", cursor: submitting ? "not-allowed" : "pointer",
            }}
          >
